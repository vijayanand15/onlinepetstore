...
@NamedQueries(
 {  @NamedQuery(
      ...
      query="SELECT i FROM com.sun.javaee.blueprints.petstore.model.Item i WHERE i.productID = :pID and i.disabled = 0"
    ), 
    @NamedQuery(
      name="Item.getAllZipCityState",
      query="SELECT z FROM com.sun.javaee.blueprints.petstore.model.ZipLocation z"       
    )
  }
) 
@Entity
