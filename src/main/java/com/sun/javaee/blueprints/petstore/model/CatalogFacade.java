...
import com.cloud.dbservice.DBServiceClient;
...
public class CatalogFacade implements ServletContextListener {
    ...
    // required: java.util.List<"+Product>
   ...
   /* static {
	   ...
	   EntityManagerFactory emf=Persistence.createEntityManagerFactory(cc);
    }*/
   ...
   public List<Category> getCategories(){
	   ...
	   try{
        ...
        categories = em.createQuery("SELECT c FROM "+Category.class.getName()+" c").getResultList();
	   }finally{
        em.close();
	   }
        return categories;
    ...
    public List<Product> getProducts(){
        ...
        EntityManager em = emf.createEntityManager();
    	try{
        products = em.createQuery("SELECT p FROM "+Product.class.getName()+" p").getResultList();
    ...
    public List<Item> getAllItemsFromCategory(String catID){
        ...
        try{
         items = em.createQuery("SELECT i FROM "+Item.class.getName()+" i, "+Product.class.getName()+" p WHERE i.productID = p.productID AND p.categoryID LIKE :categoryID AND i.disabled = 0")
        .setParameter("categoryID", catID).getResultList();
    }finally{   em.close();}
        return items;
    }
    ...
    public List<Item> getItemInChunkVLH(String pID, String iID, int chunkSize){
        ...
        Query query=null;
        try{
         query = em.createQuery("SELECT i FROM "+Item.class.getName()+" i WHERE i.productID = :pID AND i.disabled = 0");
        List<Item>  items;
    ...
    public List<Item> getItemsVLH(String pID, int start, int chunkSize){
        ...
        //Query query = em.createNamedQuery("Item.getItemsPerProductCategory");
         query = em.createQuery("SELECT i FROM "+Item.class.getName()+" i WHERE i.productID = :pID AND i.disabled = 0");
         items = query.setParameter("pID",pID).setFirstResult(start).setMaxResults(chunkSize).getResultList();
    }finally{   em.close();}
        return items;
    }
    ...
    public List<Item> getItemsByItemID(String[] itemIDs){
        ...
        if(itemIDs.length !=0) {
            ...
            String queryString = "SELECT i FROM "+Item.class.getName()+" i WHERE " +
                    "i.itemID IN (" + idString + ")  AND i.disabled = 0";
            Query query=null;
        try{
             query = em.createQuery(queryString + " ORDER BY i.name");
    ...
    public List<Item> getItemsByItemIDByRadius(String[] itemIDs, double fromLat,
        ...
        if(itemIDs.length !=0) {
            ...
            Query query=null;
            try{
             query = em.createQuery(queryString + " AND " +
    ...
    public List<Item> getItemsByCategoryVLH(String catID, int start,
        ...
        List<Item>  items;
        try{
         query = em.createQuery("SELECT i FROM "+Item.class.getName()+" i, "+Product.class.getName()+" p WHERE " +
                "i.productID=p.productID AND p.categoryID = :categoryID AND i.disabled = 0" +
                " ORDER BY i.name");
         items = query.setParameter("categoryID",catID).setFirstResult(start).setMaxResults(chunkSize).getResultList();
            }finally{   em.close();}
        return items;
    }
    /**
    ...
    public List<Item> getItemsByCategoryByRadiusVLH(String catID, int start,
        ...
        query.setParameter("fromLongitude",fromLong);
        query.setParameter("toLongitude",toLong);
         items = query.setFirstResult(start).setMaxResults(chunkSize).getResultList();
            }finally{   em.close();}
        return items;
    }
    ...
    public List<ZipLocation> getZipCodeLocations(String city, int start, int chunkSize){
        ...
        Query query=null; 
        List<ZipLocation>  zipCodeLocations=null;
        try{ query = em.createQuery("SELECT  z FROM "+ZipLocation.class.getName()+" z where UPPER(z.city) LIKE "+pattern);
         zipCodeLocations = query.setFirstResult(start).setMaxResults(chunkSize).getResultList();
    }finally{   em.close();}
    ...
    public List<Product> getProducts(String catID){
        EntityManager em = emf.createEntityManager();
        List<Product> products = em.createQuery("SELECT p FROM "+Product.class.getName()+" p WHERE p.categoryID LIKE :categoryID")
        .setParameter("categoryID", catID).getResultList();
        em.close();
        return products;
    }
    ...
    public List<Item> getItems(String prodID){
        ...
        List<Item> items =null;
        try{ 
        	items = em.createQuery("SELECT i FROM "+Item.class.getName()+" i WHERE i.productID LIKE :productID AND i.disabled = 0")
    ...
    public Item getItem(String itemID){
        EntityManager em = emf.createEntityManager();
        Item result=null;
        try{ result = em.find(Item.class,itemID);
    }finally{   em.close();}
        return result;
    }
	...
	public Collection doSearch(String querryString){
        ...
        try{
        searchQuery = em.createNativeQuery("SELECT * FROM "+Item.class.getName()+" WHERE (name LIKE ? OR description LIKE ?) AND disabled = 0" );
        searchQuery.setParameter(1, "%"+querryString+"%");
        searchQuery.setParameter(2,"%"+querryString+"%");
        
        results = searchQuery.getResultList();
    }finally{   em.close();}
        return results;
    }
    ...
    public Tag addTag(String sxTag){
        ...
        Tag tag=null;
        try {
            List<Tag> tags=em.createQuery("SELECT t FROM "+Tag.class.getName()+" t WHERE t.tag = :tag").setParameter("tag", sxTag).getResultList();
            if(tags.isEmpty()) {
    ...
    public List<Tag> getTagsInChunk(int start, int chunkSize) {
        ...
        List<Tag> tags =null;
        try{
        query = em.createQuery("FROM "+Tag.class.getName()+" t ORDER BY t.refCount DESC, t.tag");
       
         tags = query.setFirstResult(start).setMaxResults(chunkSize).getResultList();
    }finally{   em.close();}
        return tags;
    }
    ...
    public Tag getTag(String sxTag) {
        ...
        List<Tag> tags=null;
        try{
         tags=em.createQuery("SELECT t FROM "+Tag.class.getName()+" t WHERE t.tag = :tag").setParameter("tag", sxTag).getResultList();
        }finally{
         em.close();}
